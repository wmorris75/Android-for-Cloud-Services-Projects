...
public interface VideoRepository extends CrudRepository<Video, Long>{
	...
	public Video getVideoById(
			// The @Param annotation tells tells Spring Data Rest which HTTP request
			// parameter it should use to fill in the "id" variable used to
			// search for Videos
			@Param(VideoSvcApi.VIDEO_SVC_PATH + "/id") long id);
	
	
	
	//public Collection<Video> getVideoList(); //get a list of videos
...
//	public Collection<Video> findByTitle(
			...
			// parameter it should use to fill in the "id" variable used to
			// search for Videos
		//	@Param(VideoSvcApi.VIDEO_TITLE_SEARCH_PATH) String title);
	
	
	//public Collection<Video>findByDurationLessThan(
	//		@Param(VideoSvcApi.VIDEO_DURATION_SEARCH_PATH) long duration);
	
	//public Collection<String> getUsersWhoLikedVideo(
	//		@Param(VideoSvcApi.VIDEO_SVC_PATH + "/id/likedby") long duration);
